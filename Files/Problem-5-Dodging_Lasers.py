...
1,2,4,5,7
1,3,7,12,19
floor( integer + val ) = integer + floor(val)
floor(n*sqrt(2)) = floor(n + n*(sqrt(2) - 1)) = n + floor(n*(sqrt(2) - 1))

import math

def isqrt(num): ## Pass number in as long. Return the integer square root.
    num += num % 2
    res = 0;
    bit = num.bit_length()  ## The second-to-top bit is set: 1 << 30 for 32 bits
